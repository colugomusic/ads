...
TEST_CASE("mono dynamic write/read") {
	...
	count = st.write(ads::frame_count{512}, [](float* buffer, ads::frame_idx start, ads::frame_count frame_count) {
		std::iota(buffer, buffer + frame_count.value, 0.0f);
		return ads::frame_count{frame_count.value};
	});
	REQUIRE (count == ads::frame_count{512});
	count = st.read(ads::frame_count{512}, [](const float* buffer, ads::frame_idx start, ads::frame_count frame_count) {
